public class ActDefine
{
	...
	public const int	UNIT_GOT_FXETHER		=	0x61;			// GOTトランスペアレント(PC(シリアル/USB)→GOT(Ethernet)→FX3U-ENET-ADP)
	public const int	UNIT_GOT_FXENET			=	0x62;			// GOTトランスペアレント(PC(シリアル/USB)→GOT(Ethernet)→FX3U-ENET(-L))


	///ACTPROGTYPE PROTOCOLTYPE
	public const int	PROTOCOL_SERIAL			=	0x04;			// Serialﾎﾟｰﾄ経由
	public const int	PROTOCOL_USB			=	0x0D;			// USBﾎﾟｰﾄ経由
	public const int	PROTOCOL_TCPIP			=	0x05;			// TCP/IP経由
	public const int	PROTOCOL_UDPIP			=	0x08;			// UDP/IP経由
	public const int	PROTOCOL_MNETH			=	0x0F;			// Mnethﾎﾞｰﾄﾞ経由
	public const int	PROTOCOL_MNETG			=	0x14;			// Mnetgボード経由
	public const int	PROTOCOL_CCIEF			=	0x15;			// CC-Link IEフィールドボード経由
	public const int	PROTOCOL_CCLINK			=	0x07;			// CC-Link経由
	public const int	PROTOCOL_SERIALMODEM	=	0x0E;			// ｼﾘｱﾙﾎﾟｰﾄ+ﾓﾃﾞﾑ経由
	public const int	PROTOCOL_TEL			=	0x0A;			// TEL経由
	public const int	PROTOCOL_QBF			=	0x10;			// Qn用パソコンCPU経由
	public const int	PROTOCOL_QSS			=	0x11;			// Qn用SoftLogic経由
	public const int	PROTOCOL_USBGOT			=	0x13;			// USBﾎﾟｰﾄ+GOT経由
	public const int	PROTOCOL_SHAREDMEMORY	=	0x06;			// 共有ﾒﾓﾘ経由
